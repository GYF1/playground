...
Array.prototype.random = function() {
  ...
  console.log('Math.floor(Math.random()*this.length):::'+Math.floor(Math.random()*this.length));
...
Rubik.prototype.randomize = function() {
	...
	var cb = function() {
		...
		if (remain > 0) {
			...
			this._help.a = OZ.DOM.elm("p", {innerHTML:"拖动或者滑动背景可以旋转整个魔方"});
			this._help.b = OZ.DOM.elm("p", {innerHTML:"拖动或者滑动魔方块可以旋转魔方面层"});
	...
	for (var z=0;z<Rubik.SIZE;z++) {
		for (var y=0;y<Rubik.SIZE;y++) {
			for (var x=0;x<Rubik.SIZE;x++) {
				...
				//red
				if (z == 0) { cube.setFace(Face.FRONT, "#ee6557"); }

				//blue
				if (z == 2) { cube.setFace(Face.BACK, "#00aacf"); }

				//green
				if (x == 0) { cube.setFace(Face.LEFT, "#00b3a0"); }

				//yellow
				if (x == 2) { cube.setFace(Face.RIGHT, "#f6c574"); }

				//cyan
				if (y == 0) { cube.setFace(Face.TOP, "#e2e8e7"); }
